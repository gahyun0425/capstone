...
class SelfCollisionChecker:
    ...
    def __init__(
        ...
        *,
        ...
        # world checker 확인
        wc = getattr(self.robot_world, "world_coll_checker", None) or getattr(self.robot_world, "world_collision", None)
        ...
        q_test = torch.zeros(
            (1, len(self.robot_world.kinematics.joint_names)),
        ...
        for fn_name in [
            ...
            if hasattr(self.robot_world, fn_name):
                try:
                    _ = getattr(self.robot_world, fn_name)(q_test)
                    print("[WORLD-DEBUG] world distance fn works:", fn_name)
                    ok = True
                    break
        ...
        # -----------------------------
        self._joint_dist_fn_name: Optional[str] = None
        for cand in [
            "get_world_self_collision_distance_from_joints",
            "get_world_collision_distance_from_joints",
        ]:
            if hasattr(self.robot_world, cand):
                self._joint_dist_fn_name = cand
                break

        self._sphere_dist_fn_name: Optional[str] = None
        for cand in [
            "get_collision_distance",
            "get_world_collision_distance",
        ]:
            if hasattr(self.robot_world, cand):
                self._sphere_dist_fn_name = cand
                break
    ...
    def _world_self_from_joints(self, q_active: torch.Tensor) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
        if self._joint_dist_fn_name is None:
            return None, None
        fn = getattr(self.robot_world, self._joint_dist_fn_name)
        out = fn(q_active)
    ...
    def _world_from_spheres(self, x_sph: torch.Tensor) -> Optional[torch.Tensor]:
        if self._sphere_dist_fn_name is None:
            return None
        fn = getattr(self.robot_world, self._sphere_dist_fn_name)
        try:
            return fn(x_sph)
        except Exception:
            return None
    ...
    def check_batch(self, q_cspace: torch.Tensor | List[List[float]]) -> SelfCollisionBatchResult:
        ...
        d_world_tmp, _ = self._world_self_from_joints(q_active)  # 이 함수는 self.robot_world를 씀
        ...
        if d_world is None:
            ...
            d_world_s = self._world_from_spheres(x_sph_att)
    ...
    def _get_world_collision_obj(self):
        # cuRobo 버전에 따라 속성명이 다를 수 있어서 최대한 유연하게
        wc = getattr(self.robot_world, "world_coll_checker", None)
        if wc is None:
            wc = getattr(self.robot_world, "world_collision", None)
        return wc

    def set_world_obstacles_enabled(self, obstacle_names: List[str], enable: bool):
        wc = self._get_world_collision_obj()
        if wc is None:
